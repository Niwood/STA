...
class StockTradingEnv2(gym.Env):
    ...
    def _load_staged_batch(self):
        ...
        self.random_staged_batch_path = random.choice(
            glob.glob('data/staged/staged_batch_*.pkl')
            )

        # Load the batch
        with open(self.random_staged_batch_path, 'rb') as handle:
        ...
        print('A new staged batch has been loaded')
    ...
    def _next_in_batch(self):
        ...
        print(f'In batch {self.random_staged_batch_path} on iteration {self.iteration} ({self.len_this_batch})')
